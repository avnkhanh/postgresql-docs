...
In this tutorial we will backup via NFS (from vSAN, TrueNAS or Synology NAS) share and mount to /mnt
...
2. Install pgBackRest package
    ...
    === ":material-redhat: On RHEL/derivatives"
        ...
        ```{.bash data-prompt="$"}
        yum install -y percona-pgbackrest
        ```
...
1. Create environment variables to simplify the config file creation:
    ...
    ```{.bash data-prompt="$"}
	export SRV_NAME="110pgsqlha"
	export NODE1_NAME="111pgsql"
	export NODE2_NAME="112pgsql"
	export NODE3_NAME="113pgsql"
...
2. Create the `pgBackRest` repository, *if necessary*
    ...
    ```
	OR if using mount from NFS shared
    ```{.bash data-prompt="$"}
	sudo mkdir -p /mnt/backuppgsql
	sudo chown postgres:postgres /mnt/backuppgsql
	sudo chmod 750 /mnt/backuppgsql
	```

    ```{.bash data-prompt="$"}
	sudo mount -t nfs -o vers=4 vsf1.centersl.local:/vsanfs/backuppgsql /mnt/backuppgsql
	echo "vsf1.centersl.local:/vsanfs/backuppgsql /mnt/backuppgsql nfs vers=4,defaults 0 0" | sudo tee -a /etc/fstab
	```	
...
3. The default `pgBackRest` configuration file location is `/etc/pgbackrest/pgbackrest.conf`, but some systems continue to use the old path, `/etc/pgbackrest.conf`, which remains a valid alternative. If the former is not present in your system, create the latter.
    ...
    === ":material-debian: On Debian/Ubuntu"
		...
		[global]
		# Repository (backup storage) details
		repo1-path=/mnt/backuppgsql
		...
		pg1-socket-path=/etc/postgresql/unix_socket
		...
		pg2-socket-path=/etc/postgresql/unix_socket
		...
		pg3-socket-path=/etc/postgresql/unix_socket
		EOF
...
8. If you are using Patroni, change its configuration to use `pgBackRest` for archiving and restoring WAL files. Run this command only on one node, for example, on `111pgsql`: 
